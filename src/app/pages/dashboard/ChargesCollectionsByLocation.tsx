...
const getLocationName = `/api/ChargesByLocation/getLocationName`
...
const ChargesCollectionsByLocation: React.FC = () => {
  ...
  if(PieChartData.length > 0){
    Result = PieChartData.map((res: any, _inde: any) => {
      ...
      var FilterDataCountClaims :any = GetCountByPosttedTable.filter((x:any)=> x.facility == res.facility)
      var FilterData :any = PaymentsData.filter((x:any)=> x.facility == res.facility)
      ...
      GeneeratePieChartData = [res.facility,res.charges,Payments]
      ...
      if(_inde == PieChartData.length - 1){
          ...
          return (
              ...
              <tr>
                <td >{res.facility}</td>
                <td >{FilterDataCountClaims[0].count}</td>
                <td >{currencyFormatter.format(res.charges)}</td>
                <td >{currencyFormatter.format(Payments)}</td>
                <td >{Number(Number(res.charges) / Number(Payments)).toFixed(2)}%</td>
              </tr>
              <tr>
                <td >Grand Total</td>
                <td >{TotalClaims}</td>
      ...
      <div >
        ...
        >
          <div className='card-title m-0'>
            <h3 className='fw-bolder m-0'>Charges and Collections By Location</h3>
          </div>
        </div>
        <hr />
        <div className='row'>
            ...
            <>
              ...
              <div className='col-sm-4 fv-row'>
                <label className='col-lg-6 col-form-label fw-bold fs-6'>
                  <span className=''>Location List</span>
                </label>
        ...
        <div className='row'>
          <div className='col-12'>
            ...
            {Result ? <Chart chartType="Bar" data={TotalListChart} options={{
            ...
            <div className="card shadow-sm">
              {/* <div className="card-header">
               ...
               {Result ? <div className="card-body">
                <ul>
                  <li>Major Contributions are from xxx – 85.56%, xx – 14.19%. And Other locations contributed 0.25% on the overall Collections ($33M)</li>
