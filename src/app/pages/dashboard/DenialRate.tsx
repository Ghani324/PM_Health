...
const[RejectRateTable,setRejectRateTable]=useState<any>([])
  ...
  const FilterDataList = async () => {
    ...
    setOpen(true)
    axios.get(`${getDenialRate}?FromDate=${DateMonthFrom}&ToDate=${DateMonthTo}&PracticeName=${SelectedPracticeList.length > 0 ? JSON.stringify(SelectedPracticeList) : ""}`).then(_res => {
      setOpen(false)
      ...
      <div>
        ...
        >
          <div className='card-title m-0'>
            <h3 className='fw-bolder m-0'>KPI's - Denial Rate</h3>
          </div>
        </div>
        <hr />
        <div className='row'>
                ...
                <tbody>
                  {RejectRateTable.map((res:any,ind:any)=>{
                    return (
                      ...
                      <td className='px-3 py-2 text-center'>{res.Denial_Charges ? res.Denial_Charges.toLocaleString('en-US', options) : "None"}</td>
                      <td className='px-3 py-2 text-center'>{res.Total_Charges ? res.Total_Charges.toLocaleString('en-US', options) : "None"}</td>
                      <td className='px-3 py-2 text-center'>{res.Denial_Rate ? res.Denial_Rate : "-"}</td>
                   </tr>
                    )
                  })}
                </tbody>
