...
const[RejectRateTable,setRejectRateTable]=useState<any>([])
  ...
  const FilterDataList = async () => {
    ...
    setOpen(true)
    axios.get(`${getRejectionRate}?FromDate=${DateMonthFrom}&ToDate=${DateMonthTo}&PracticeName=${SelectedPracticeList.length > 0 ? JSON.stringify(SelectedPracticeList) : ""}`).then(_res => {
      setOpen(false)
      ...
      <div>
        ...
        >
          <div className='card-title m-0'>
            <h3 className='fw-bolder m-0'>KPI's - Rejection Rate</h3>
          </div>
        </div>
        <hr />
        <div className='row'>
