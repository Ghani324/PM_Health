...
const getClaims = `/api/PCA/getClaims`
const GetClaimOutstanding = `/api/PCA/GetClaimOutstanding`
const getUserList = `/users/getUsersList`
const getClaimHistory = `/api/PCA/getClaimHistory`
const ClaimUpdateUrl = `/api/PCA/ClaimUpdate`
const AssignClaimUser = `/api/PCA/AssignClaimUser`
const getPracticeList = `/api/practice/getPracticeListById`
const DeleteClaimHistoryById = `/api/PCA/DeleteClaimHistoryById`
...
const PCA: React.FC = () => {
    ...
    const handleExport = async (_e: any) => {
        ...
        var url = `${getClaims}?pmid=${JSON.stringify(SelectedPmSystem)}&practicelist=${JSON.stringify(SelectedPractice)}&page=${pageState.pageSize.page}&limit=${pageState.pageSize.pageSize}&filter=${JSON.stringify(filterModel.items)}&type=download&sort=${JSON.stringify(sortModel)}`
    ...
    const fetchData = async (GetData: any) => {
        ...
        if (userDetails) {
            console.log("Data PCA",userDetails)
            ...
            console.log("userDetails.PageType", userDetails.SelectedData[0].filterModel)
            navigate("/PCA", { replace: true })
        }
    ...
    const getSelectedColumns = async () => {
        const getUserColumns = await axios.get(`${getColumnsByUser}?PageType=PCA`)
    ...
    const columns: GridColDef[] = [
        ...
        { align: 'center', headerAlign: "center", field: 'PCAStatus', headerName: 'PCA Status', width: 100, renderCell: ({ row }: Partial<GridRowParams>) =>
        <a href="#" className='' onClick={() => navigateViewClaimbyAccount(row)}>
            {row.PCAStatus === "Yes" ? "Yes" : "No" }
        </a> },
        ...
        { align: 'center', headerAlign: "center", field: 'SecondaryInsuranceCompany', headerName: 'Secondary Insurance Company', width: 200, },
        { align: 'center', headerAlign: "center", field: 'DOE', headerName: 'DOE', width: 200, },
        { align: 'center', headerAlign: "center", field: 'FirstName', headerName: 'Caller Name', width: 200, renderCell: ({ row }: Partial<GridRowParams>) => <>{row.FirstName ? row.FirstName + " " + row.LastName : ""}</> },
        { align: 'center', headerAlign: "center", field: 'CallerStatusChanged', headerName: 'Status Changed', width: 200, },
        { align: 'center', headerAlign: "center", field: 'CallerStatusChangedDate', headerName: 'Status Changed Date', type: "text", width: 200, renderCell: ({ row }: Partial<GridRowParams>) => <>{row.CallerStatusChangedDate ? moment(row.CallerStatusChangedDate).format("MM-DD-YYYY") : null }</> },
        { align: 'center', headerAlign: "center", field: 'RootCause', headerName: 'Root Cause', width: 200, },
        { align: 'center', headerAlign: "center", field: 'StatusUpdatedbyBOT', headerName: 'Status Updated By BOT', width: 200, },
        ...
        { filterOperators: DateOperator,align: 'center', headerAlign: "center", field: 'DueDate', type: 'text', headerName: 'Due Date', width: 200, renderCell: ({ row }: Partial<GridRowParams>) => <>{row.DueDate ? moment(row.DueDate ).tz("America/florida").format("MM-DD-YYYY") : null}</> },

        {
            align: 'center', headerAlign: "center", field: 'Comments', headerName: 'Latest Comments', width: 200, renderCell: ({ row }: Partial<GridRowParams>) =>
    ...
    const AssignClaims = async() => {
        ...
        var StatusCodesCheck = ["CALL","Fresh-Call","RECALL"]
        ...
        if(CheckEmpty.length > 0){
            swal({
                title: '',
                icon: "info",
                text: `Status Must be Call / Fresh-Call / RECALL`,
            }).then((_res: any) => {
    ...
    const setColumnVisibilityModel = (e:any) =>{
        setDefaultColumns(e)
        axios.post(UpdateColumnsByUser, { Columns: JSON.stringify([e]) ,PageType : "PCA"}).then((res) => {
        ...
        <div >
            ...
            >
                <div className='card-title m-0'>
                    <h3 className='fw-bolder m-0'>Post Call Analysis Report</h3>
                </div>
            </div>
            <hr />
            <div className='row'>
                ...
                <div className='col-1 pt-13'>
                    ...
                    <div className='row'>
                        {/* <br/>
                        <hr /> */}
                        <p className="h4 text-center mt-6 mb-6">Assign Claim</p>
